...
def insert_bank_record(conn, engine: str, record: dict) -> int:
    ...
    values = (
        record["ci_cipher"],
        record["nombres_cipher"],
        record["apellidos_cipher"],
        record["numero_cuenta_cipher"],
        record["tipo_cuenta_cipher"],
        record["saldo_usd_cipher"],
        record["saldo_bs_cipher"],
    ...
    for banco_id, cfg in BANKS.items():
        ...
        try:
            for _, row in bank_df.iterrows():
                ...
                bank_record = {
                    "ci_cipher": encrypt(str(identificacion)),
                    "nombres_cipher": encrypt(str(nombres)),
                    "apellidos_cipher": encrypt(str(apellidos)),
                    "numero_cuenta_cipher": encrypt(str(nro_cuenta)),
                    "tipo_cuenta_cipher": encrypt(tipo_cuenta),
                    "saldo_usd_cipher": encrypt(str(saldo)),
                    "saldo_bs_cipher": "",  # por ahora vacío
